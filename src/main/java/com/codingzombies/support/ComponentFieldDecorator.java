...
class ComponentFieldDecorator extends DefaultFieldDecorator {
    ...
    protected <T> List<T> proxyForListLocator(SearchContext searchContext, Class<T> clazz, ClassLoader loader, ElementLocator locator) {
        ...
        return proxy.stream().map(element -> {
            return ClassSupport.newInstance(clazz, (WebDriver)driver, element);
